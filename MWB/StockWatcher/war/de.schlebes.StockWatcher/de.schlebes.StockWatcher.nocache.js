...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "de.schlebes.StockWatcher" because\n';
  ...
  $wnd.__gwt_activeModules['de.schlebes.StockWatcher'] = {
    'moduleName' : 'de.schlebes.StockWatcher',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/de.schlebes.StockWatcher.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['de.schlebes.StockWatcher'].superdevmode = true;

  var expectedSuffix = '/de.schlebes.StockWatcher.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + de.schlebes.StockWatcher + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:de.schlebes.StockWatcher';
  var devModeSessionKey = '__gwtDevModeSession:de.schlebes.StockWatcher';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/de.schlebes.StockWatcher/de.schlebes.StockWatcher.nocache.js';
  ...
  $wnd.__gwt_activeModules['de.schlebes.StockWatcher'].canRedirect = true;
  $wnd.__gwt_activeModules['de.schlebes.StockWatcher'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING de.schlebes.StockWatcher';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/de.schlebes.StockWatcher?user.agent=' + ua + '&_callback=' + callback;

...
public class StockWatcher implements EntryPoint {
	...
	private void addStock() {
		...
		// or dots.
		if (!symbol.matches("^[0-9A-Z&#92;&#92.]{1,10}$")) {
	...
	private void refreshWatchList() {
		/*
		 * final double MAX_PRICE = 100.0; // $100.00 final double
		 * MAX_PRICE_CHANGE = 0.02; // +/- 2%
		 * 
		 * StockPrice[] prices = new StockPrice[ stocks.size()]; for ( int i =
		 * 0; i < stocks.size(); i++) { double price = Random.nextDouble() *
		 * MAX_PRICE; double change = price * MAX_PRICE_CHANGE * (

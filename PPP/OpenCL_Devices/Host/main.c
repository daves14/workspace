 ...
 *
 *  gcc -o cldemo -std=gnu99 -Wall -lOpenCL -I/usr/include/nvidia-current cldemo.c
 *
 */
/* Modified by Kees Verstoep, VU University, to allow runtime platform selection */
...
int main(int argc, char **argv)
{
...
#else
  if (getenv("FORCE_GPU") != NULL) {
    CL_CHECK(clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_GPU, 100, devices, &devices_n));
  } else if (getenv("FORCE_CPU") != NULL) {
    CL_CHECK(clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_CPU, 100, devices, &devices_n));
  } else if (getenv("FORCE_ACCELERATOR") != NULL) {
    CL_CHECK(clGetDeviceIDs(platforms[0], CL_DEVICE_TYPE_ACCELERATOR, 100, devices, &devices_n));
  } else {
#ifdef ORIG
  ...
  if (getenv("FORCE_DEVICE") != NULL) {
    device_no = atoi(getenv("FORCE_DEVICE"));
  }

  if (devices_n == 0 || device_no >= devices_n)
    return 1;
  printf("Using device %d\n", device_no);

  cl_context context;
  context = CL_CHECK_ERR(clCreateContext(NULL, 1, &devices[device_no], &pfn_notify, NULL, &_err));
  ...
  if (clBuildProgram(program, 1, &devices[device_no], "", NULL, NULL) != CL_SUCCESS) {
    char buffer[10240];
    clGetProgramBuildInfo(program, devices[device_no], CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, NULL);
  ...
  queue = CL_CHECK_ERR(clCreateCommandQueue(context, devices[device_no], 0, &_err));
